...
from config.system_prompt import SUMMARY_SCORE_SYSTEM_PROMPT,SUMMARY_SCORE_USER_PROMPT
...
def get_annotator_correction_prompt(data, type):
    ...
    if(type == "safety"):
        ...
        answer = str(data.get('answer', data.get('original_answer', data.get('response_0'))))
        correction = str(data.get('correction', data.get('correction_answer', data.get('response_1'))))
        ...
        answer = str(data['answer'])
        correction = str(data['correction'])
        user_prompt = SUMMARY_SCORE_USER_PROMPT.format(
            question=question,
            responseA=answer,
            responseB=correction,
        )
        return SUMMARY_SCORE_SYSTEM_PROMPT, user_prompt
    elif(type == "math"):
        question = str(data['question'])
        answer = str(data['answer'])
...
def transform_data(data, type):
    ...
    if type == "math":
        ...
        new_data['answer'] = data.get('correction', data.get('output', data.get('last', data.get('model_output'))))
    ...
    new_data["question"] = data.get('prompt', data.get('question'))
    new_data["answer"] = data.get('original_answer', data.get('answer', data.get('response_0')))
    new_data["correction"] = data.get('correction_answer', data.get('correction', data.get('output', data.get('response_1'))))
    # if order == 0:
    #     new_data["answer"] = data.get('original_answer', data.get('answer'))
    #     new_data["correction"] = data.get('output', data.get('correction'))
    # else:
    #     new_data["answer"] = data.get('output', data.get('correction'))
    #     new_data["correction"] = data.get('original_answer', data.get('answer'))
    new_data['order'] = order
    return new_data, order
