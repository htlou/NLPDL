...
def bean_gpt_api(
    ...
    session.mount("http://", adapter)
    max_try = 2
    # logging.error(params_gpt)
    while max_try > 0:
        try:
            response = session.post(url, headers=headers, data=json.dumps(params_gpt), timeout=(25, 300))
            if response.status_code == 200:
                response = json.loads(response.text)['choices'][0]['message']['content']
                # print(response)
                logging.info(response)
            ...
            err_msg = f'Access openai error, Key platform Bean, Key id: {openai_api_keys}, status code: {"N/A"}, status info : {"N/A"}, Error: {str(e)}'
            logging.error(err_msg)
            max_try -= 1
    else:
