...
for NUM_SHOT in ${NUM_SHOTS[@]}; do
	TRAIN_DATASETS="${TRAIN_DATASETS_ROOT}/preference_train_${NUM_SHOT}.json"
	OUTPUT_DIR="${ROOT_DIR}/output/dpo/preference_${NUM_SHOT}"
	mkdir -p "${OUTPUT_DIR}"
	OUTPUT_DIR="$(cd "${OUTPUT_DIR}" &>/dev/null && pwd)"
	deepspeed "${DEEPSPEED_ARGS[@]}" \
		--module safe_rlhf.algorithms.dpo \
        --train_datasets nlpdl-preference::${TRAIN_DATASETS} \
